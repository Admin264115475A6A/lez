...
# Thread-safe lock for JSON file updates
...
# Function to write block file names to a JSON file (with thread-safety)
def write_block_files_to_json():
    try:
        ...
        logging.error(f"Error writing block file names to JSON: {e}")

# Function to check for shutdown signal
...
# Function to generate seeds and hashes
...
while True:
    ...
    if check_for_shutdown_signal(shutdown_file):
        print("Shutdown signal detected. Exiting safely...")
        break

    # Generate and write block
